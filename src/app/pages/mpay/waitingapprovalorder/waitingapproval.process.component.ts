...
import { OrderWorkflowRequest, OrderMessageGetByOrderIdRequest, AuctionCheckDeletedBidRequest } from 'src/app/models/request/order.request';
...
@Component({
    ...
    selector: 'mpay-waiting-approval-process',
    templateUrl: './waitingapproval.process.component.html',
    styleUrls: ['./waitingapproval.process.component.scss']
})


export class WaitingApprovalProcessComponent implements OnInit {
    ...
    private checkDeletedBidEvent: EventEmitter<any> = new EventEmitter<any>();
    ...
    constructor(public activeModal: NgbActiveModal,
        private orderService: WaitingApprovalOrderService, private modalService: NgbModal,
    ...
    ngOnInit() {
        ...
        this.onCheckDeletedBidListener();
    }
    ...
    private onCheckDeletedBidListener(): void {
        merge(this.checkDeletedBidEvent)
            .pipe(
                switchMap(() => {
                    const request: AuctionCheckDeletedBidRequest = {
                        orderCode: this.order.code
                    };

                    return this.orderService.checkShopDeletedBid(request);
                }),
                catchError(() => {
                    this.utility.showError(ErrorCodeDefine.UNKNOW_ERROR, null);
            ...
            .subscribe(response => {
                if (!response.status) {
                    this.utility.showErrorMessage("Không thể lấy thông tin!");

                    return;
                } 
                
                if(!response.data) {
                    this.utility.showErrorMessage("Không tìm thấy thông tin bị shop huỷ!");

                    return;
                }
                
                this.utility.showMessage("Đơn hàng đã bị shop huỷ!");
            });
    }
    ...
    addPaymentOpenModel(): void {
        const modalRef = this.modalService.open(WaitingApprovalAddPaymentComponent);
    ...
    checkDeletedBid(): void {
        this.checkDeletedBidEvent.emit();
    }
}
