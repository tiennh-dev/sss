...
import { WorkflowTranslateGraph, WithdrawalWorkflowTranslateGraph } from 'src/app/common/config-setting';
...
export class WorkflowWithdrawalComponent implements OnInit {
    @Input() itemId: any;
    ...
    ngOnInit() {
        this._workflowService.dotWithHistory('WITHDRAWAL_WORKFLOW', this.itemId).subscribe((dot: string) => {
            if (dot === 'NOT FOUND') {
                this.check = true;
                this.message = 'Quy trình chưa được khởi tạo';
            }
            dot = WithdrawalWorkflowTranslateGraph.translate(dot);
            this.dot = { dot: dot };
        });
    }
}