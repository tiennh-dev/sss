...
export class IndexComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    private ctService: DepositsService,
    ...
    this.permissionService.ResourceKey="DEPOSITS";
    this.search_state = [];
    this.pageLength = 50;
  }
  ...
  ngOnInit() {
    ...
    tblToolbar.push({
      text: '<i  class="icofont icofont-ui-add"></i> Nạp tiền',
      key: 'a', shiftKey: true,
              ...
              return {
                callback: function (key, options) {
                  if (key === 'confirm-status') {
                    ...
                    const modalRef = self.modalService.open(ConfirmStatusComponent);
            ...
            build: function ($triggerElement, e) {
              return {
                callback: function (key, options) {
                  ...
                  // }
                  if (key === 'Process') {
                    const modalRef = self.modalService.open(TransactionProcessingdepComponent, { size: 'lg' });
                    modalRef.componentInstance.itemId = data.Id;
                  ...
                  if (key === 'workflow') {
                    const modelRef = self.modalService.open(WorkflowDepositsComponent, { size: 'lg' });
  ...
  exportExcel():void{
    ...
    .pipe(
      ...
      let filename = 'DepositData_'+myDate+'.xlsx';
