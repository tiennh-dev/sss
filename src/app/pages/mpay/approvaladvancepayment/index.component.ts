...
import { OrderState, OrderStatus, OrderRefType, MenuContextDefine2, PermissionCommon, Actions } from 'src/app/common/config-setting';
...
@Component({
  selector: 'app-mpay-approval-advance-payment',
...
export class IndexComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.permissionService.ResourceKey="APPROVALADVANCEPAYMENT";
    this.search_status = []; //1, 4, 7
    this.search_state = [OrderState.CHO_XU_LY_TAM_UNG];
  ...
  ngOnInit() {
    ...
    tblToolbar.push({
      text: '<span data-action-target="button" data-action='+Actions.DUYET_TAMUNG+'><i  class="icofont icofont-ui-add"></i>Duyệt tạm ứng</span>',
          key: 'a', shiftKey: true,
          action: (e, dt, node, config) => {
            if(this.lstOrderId.length == 0){
              return this.utility.showError('Bạn chưa chọn đơn hàng!');
            }
            const modalRef = this.modalService.open(AcceptApprovalPaymentMultiComponent, { size: 'lg' });
            modalRef.componentInstance.listOrderId= this.lstOrderId;
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        },
        {
          ...
          }, orderable: false, visible:false,className: 'mw120'
        },
        {
          data: 'PaymentDurationSeconds', title: 'Hạn thanh toán', render: function (data: any, type: any, item: any) {
            return '<span class="count_down" data-endtime="' + data + '"></span>';
      ...
      rowCallback: (r: Node, data: model.OrderJTable) => {
        $('.count_down', r).each((ielem, elem) => {
          var s = $(elem).data('endtime');
          var x = setInterval(function () {
            var m = Math.floor(s / 60);
            var h = Math.floor(m / 60);
            var d = Math.floor(h / 24);
            var _h = h - (d * 24); var _m = m - (h * 60); var _s = s - (m * 60);
            var str = d + "d - " + _h + "h " + _m + "' " + _s + "s ";
            $(elem).html(str);
            s = s - 1;
          }, 1000);
        });
  ...
  openProcess(id: any): void {
    const modalRef = this.modalService.open(ApprovalAdvancePaymentProcessComponent, { size: 'lg' });
      ...
      build: function ($triggerElement, e) {
        return {
          callback: function (key, options) {
            ...
            if (key === 'customer') {
              const modelRef = self.modalService.open(ApprovalAdvanceCustomerInfoComponent, { size: 'lg' });
           ...
           if (key === 'workflow') {
             const modelRef = self.modalService.open(WorkflowAdvancePaymentComponent, { size: 'lg' });
