...
@Component({
  ...
  providers: [NgbModalConfig, NgbModal, CustomerService,DatePipe],
})
...
export class IndexComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    private orderService: ApprovalOrderService,
    private permissionService: PermissionService,
    private datePipe: DatePipe,
  ) {
    this.permissionService.ResourceKey="APPROVALORDER";
    this.search_status = []; //1, 4, 7
    this.search_state = [OrderState.CHO_XU_LY_TAM_UNG, OrderState.CHO_XU_LY_HUY_DON, 
      OrderState.CHO_XL_DE_NGHI_TT, OrderState.CHO_TT_DON_HANG];
  ...
  ngOnInit() {
    ...
    tblToolbar.push({
      text: '<span data-action-target="button" data-action='+Actions.DUYET_TAMUNG+'><i  class="icofont icofont-ui-add"></i>Duyệt tạm ứng</span>',
          ...
          action: (e, dt, node, config) => {
            ...
            const modalRef = this.modalService.open(AcceptApprovalMultiComponent, { size: 'lg' });
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        {
          data: 'EmpployeeSupport', title: 'Nhân viên hỗ trợ',
        }
      ],
  ...
  openProcess(id: any): void {
    const modalRef = this.modalService.open(ProcessComponent, { size: 'lg' });
      ...
      build: function ($triggerElement, e) {
        return {
          callback: function (key, options) {
           ...
           if (key === 'workflow') {
             const modelRef = self.modalService.open(WorkflowComponent, { size: 'lg' });
  ...
  htmlProductInfo(data: model.OrderDetailPayment, orderData: model.OrderJTable): string {
    ...
    result +=`<div class="card">
            ...
            data.orderdetails.forEach(element => {
              let htmlLink = '';
              if (orderData.OrderType === "AUCTION") {
                htmlLink = "https://ichibajp.com/yahoo-auction/" + element.productLink;
              } else {
                htmlLink = element.productLink;
              }
            result += `<tr>
                        <td><a target="_blank" href="${element.previewImage}"><img class="img-list img-fluid" src="${element.previewImage}"/></a></td>
                        <td><a href=`+ htmlLink + ` target="_blank">${element.productName}</a></td>
                        <td>${element.ref}</td>
    ...
    exportExcel(): void {
      merge()
        .pipe(
          ...
          let filename = 'Thanh Toán Đơn Hàng_'+myDate+'.xlsx';
