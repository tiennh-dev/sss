...
export class IndexComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    private ctService: WaitingDepositsService,
    ...
    this.permissionService.ResourceKey="WAITINGDEPOSITE";
    this.search_state = [];
    this.pageLength = 50;
  }
    ...
    tblToolbar.push({
      text: '<span data-action-target="button" data-action='+Actions.ACCESS+'><i  class="icofont icofont-ui-add"></i> Nạp tiền</span>',
          ...
          action: (e, dt, node, config) => {
            const modalRef = this.modalService.open(CreateDepositeComponent);
              ...
              return {
                callback: function (key, options) {
                  if (key === 'confirm-status') {
                    ...
                    const modalRef = self.modalService.open(WaitingDepositeConfirmStatusComponent);
            ...
            build: function ($triggerElement, e) {
              return {
                callback: function (key, options) {
                  ...
                  // }
                  if (key === 'Process') {
                    const modalRef = self.modalService.open(WaitingDepositeProcessingdepComponent, { size: 'lg' });
                    modalRef.componentInstance.itemId = data.Id;
                  ...
                  if (key === 'workflow') {
                    const modelRef = self.modalService.open(WorkflowWaitingDepositsComponent, { size: 'lg' });
                  ...
                  if (key === 'customer') {
                    const modelRef = self.modalService.open(WaitingDepositeCustomerInfoComponent, { size: 'lg' });
  ...
  exportExcel():void{
    ...
    .pipe(
      ...
      let filename = 'WaitingDepositData_'+myDate+'.xlsx';
