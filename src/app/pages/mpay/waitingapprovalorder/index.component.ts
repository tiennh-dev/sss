...
export class IndexComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.permissionService.ResourceKey="WAITINGAPPROVALORDER";
    this.search_status = []; //1, 4, 7
    this.search_state = [OrderState.CHO_XU_LY_HUY_DON];
      ...
      columns: [
        {
          ...
          render: function (data: any, type: any, item: any) {
            if (item.State === OrderState.CHO_XU_LY_HUY_DON) {
              return `
  ...
  openProcess(id: any): void {
    const modalRef = this.modalService.open(WaitingApprovalProcessComponent, { size: 'lg' });
      ...
      build: function ($triggerElement, e) {
        return {
          callback: function (key, options) {
            ...
            if (key === 'customer') {
              const modelRef = self.modalService.open(WaitingApprovalCustomerInfoComponent, { size: 'lg' });
           ...
           if (key === 'workflow') {
             const modelRef = self.modalService.open(WaitingApprovalWorkflowComponent, { size: 'lg' });
                                  ...
                                  result +=
                                          `<tr>
                                                ...
                                                item.status === 0 ? '<span class="badge badge-warning">Chờ xử lý</span>' :
                                                  ...
                                                  item.status === 3 ? '<span class="badge badge-info">Chờ thanh toá</span>' : ''}
                                                </td>
                                                <td>
